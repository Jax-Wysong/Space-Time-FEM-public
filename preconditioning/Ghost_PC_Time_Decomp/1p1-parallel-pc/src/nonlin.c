  ...
  */	
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
			...
			// global coords for forcing function, ensure periodic wrap
			PetscReal L = xR - xL;
			...
			PetscScalar forcing_phi = lam22 * (phixt + (-2.0) * lam22 * phixt * A_f * PetscPowScalar(B_f, -1.5));
			PetscScalar forcing_chi = lam22 * (chixt - 2.0 * lam22 * chixt * C_f * PetscPowScalar(B_f, -1.5));
			  			
			/* we subtract the forcing terms since they are on the RHS and we want them on the LHS */
            for (int a = 0; a < 4; ++a){
