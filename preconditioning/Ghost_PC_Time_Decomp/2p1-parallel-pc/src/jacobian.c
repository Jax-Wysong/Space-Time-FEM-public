...
PetscErrorCode FormJacobian(SNES snes, Vec U, Mat J, Mat P, void *ctx) {
      ...
      for (PetscInt xElm = xs; xElm < xs+xm;   ++xElm) {
        ...
        for (int i=0;i<8;++i){
            ...
            for (int j=0;j<8;++j){
                ...
                M[r1][cphi] += (A_space_x[i][j] + A_space_y[i][j]) + A_mass[i][j];
                M[r1][cu]   += A_time [i][j];
                M[r2][cphi] += A_time [i][j];
                M[r2][cu]   += -A_mass [i][j];

                M[r3][cchi] += (A_space_x[i][j] + A_space_y[i][j]) + A_mass[i][j];
                M[r3][cv]   += A_time [i][j];
  ...
  MatZeroRowsColumnsStencil(J, nlocal, bc, 1.0, NULL, NULL);
  if (P != J) MatZeroRowsColumnsStencil(P, nlocal, bc, 1.0, NULL, NULL);
