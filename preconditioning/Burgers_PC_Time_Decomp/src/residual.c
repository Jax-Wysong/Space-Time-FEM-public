...
PetscErrorCode FormResidual(SNES snes, Vec U, Vec R, void *ctx)
{
  ...
  for (PetscInt tElm = ys; tElm <=tEnd; ++tElm) {            /* nt-1 elements in t */
    for (PetscInt xElm = xs; xElm <xs+xm;   ++xElm) {          /* all owned x (periodic DMDA handles) */
      ...
      PetscScalar eqn1_r_local[4]={0}; /* 1 eqn in Burgers weak form */
      /* linear part from constant-coeff matrices */
      for (PetscInt i=0;i<4;++i){
        for (PetscInt j=0;j<4;++j){
          eqn1_r_local[i] += A_time[i][j]*u_local[j] + (user->viscosity)*A_space[i][j]*u_local[j];
        }
      }

      
      PetscScalar eqn1_r_nonlin[4];
	  /* This problem is nonlinear, so we need this to compute the nonlinear terms */
    PetscReal x0 = xL + xElm*hx;
		PetscReal t0 = user->t0 + tElm*ht;	  
  ...
  if (ys==0){
    ...
    PetscScalar (**ug)[1];                        /* Ul is still valid and ghosted*/
    ...
    if(user->IC == 1)
    {
      for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
        ...
        rg[0][x][0] = ug[0][x][0] - Burgers_IC_u(x_phys, user->viscosity);
      }
    }	
