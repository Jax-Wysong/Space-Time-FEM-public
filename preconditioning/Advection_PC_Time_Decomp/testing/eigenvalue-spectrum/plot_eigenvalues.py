...
for ax, c in zip(axes_J.flat, cases):
    ...
    at_one_J   = ev[np.abs(ev - 1) < 1e-6]
    polluted_J = ev[np.abs(ev - 1) >= 1e-6]
    ...
    leg1 = ax.legend(handles=handles + stat_handles(kappa),
                     fontsize=LEGEND_FS, loc='upper left')
    ax.add_artist(leg1)

    count_handles = [
        mlines.Line2D([], [], linestyle='none', color='none',
                      label=f'λ≈1:  {len(at_one_J)}'),
        mlines.Line2D([], [], linestyle='none', color='none',
                      label=f'polluted:  {len(polluted_J)}'),
    ]
    ax.legend(handles=count_handles, fontsize=LEGEND_FS, loc='lower left')
