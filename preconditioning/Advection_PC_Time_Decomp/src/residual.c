    ...
    for (PetscInt xElm = xs; xElm <xs+xm;   ++xElm) {          /* all owned x (periodic DMDA handles) */
      ...
      for (PetscInt i=0;i<4;++i){
        for (PetscInt j=0;j<4;++j){
          eqn1_r_local[i] += A_time[i][j]*u_local[j] + (user->c)*A_space[i][j]*u_local[j];
        }
      }

      
      // PetscScalar eqn1_r_nonlin[4];
	  /* This problem is linear, so we don't need this to compute the nonlinear terms */
  ...
  if (ys==0){
    ...
    if(user->IC == 1)
    {
      for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
        ...
        rg[0][x][0] = ug[0][x][0] - Gaussian_IC_u(x_phys);
      }
    }	
