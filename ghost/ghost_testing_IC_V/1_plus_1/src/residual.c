...
PetscErrorCode FormResidual(SNES snes, Vec U, Vec R, void *ctx)
{
  ...
  for (PetscInt tElm = ys; tElm <=tEnd; ++tElm) {            /* nt-1 elements in t */
    for (PetscInt xElm = xs; xElm <xs+xm;   ++xElm) {          /* all owned x cols (periodic, dm handles)  */
      ...
      ComputeR_local_nonlinear(eqn1_r_nonlin,eqn3_r_nonlin,phi_local,chi_local,hx,ht,ctx);
      for (PetscInt i=0;i<4;++i){
  ...
  if (ys==0){
	...
	if(IC == 1)
	{
		for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
			...
			rg[0][x][0] = ug[0][x][0] - phi_wave_IC(x_phys,A,user->C);
			rg[0][x][1] = ug[0][x][1] - u_wave_IC  (x_phys,mphi2,A,user->C);
			rg[0][x][2] = ug[0][x][2] - chi_wave_IC(x_phys,A,user->C);
			rg[0][x][3] = ug[0][x][3] - v_wave_IC  (x_phys,mchi2,A,user->C);
		}
	}
	...
	if(IC == 2)
	{
		for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
			PetscReal x_phys = xL + x*hx;
			rg[0][x][0] = ug[0][x][0] - phi_gauss_IC(x_phys,A,user->C);
			rg[0][x][1] = ug[0][x][1] - u_gauss_IC  (x_phys,mphi2,A,user->C);
			rg[0][x][2] = ug[0][x][2] - chi_gauss_IC(x_phys,A,user->C);
			rg[0][x][3] = ug[0][x][3] - v_gauss_IC  (x_phys,mchi2,A,user->C);
		}
	}
	...
	if(IC == 3)
	{
		for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
		  PetscReal x_phys = xL + x*hx;
		  rg[0][x][0] = ug[0][x][0] - phi_noise_IC(x_phys, A, user->ns,
												   user->n1,user->n2,user->L, user->theta);
		  rg[0][x][1] = ug[0][x][1] - u_noise_IC  (x_phys, mphi2, A, user->ns,
												   user->n1,user->n2,user->L, user->theta, user->ssign);
		  rg[0][x][2] = ug[0][x][2] - chi_noise_IC(x_phys, A, user->ns,
												   user->n1,user->n2,user->L, user->theta, user->dphi);
		  rg[0][x][3] = ug[0][x][3] - v_noise_IC  (x_phys, mchi2, A, user->ns,
	...
	if (IC == 4) 
	{
	  for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
		PetscReal x_phys = xL + x*hx;
		rg[0][x][0] = ug[0][x][0] - phi_pw_IC(x_phys, A);
		rg[0][x][1] = ug[0][x][1] - u_pw_IC  (x_phys, mphi2, A);
		rg[0][x][2] = ug[0][x][2] - chi_pw_IC(x_phys, A, user->pw_r, user->pw_dphi);
		rg[0][x][3] = ug[0][x][3] - v_pw_IC  (x_phys, mchi2, A, user->pw_r, user->pw_dphi, user->pw_sigma);
	  }
	}
	...
	for (PetscInt x = info.xs; x < info.xs + info.xm; ++x) {
		PetscReal x_phys = xL + x*hx;
		rg[0][x][0] = ug[0][x][0] - phi_osc_IC(x_phys, A,
											   user->osc_x0, user->osc_sigma, user->osc_k0);
		rg[0][x][1] = ug[0][x][1] - u_osc_IC  (x_phys, mphi2, A,
											   user->osc_x0, user->osc_sigma, user->osc_k0);
		rg[0][x][2] = ug[0][x][2] - chi_osc_IC(x_phys, A, user->osc_r,
											   user->osc_x0, user->osc_sigma, user->osc_dphi, user->osc_k0);
		rg[0][x][3] = ug[0][x][3] - v_osc_IC  (x_phys, mchi2, A, user->osc_r,
											   user->osc_x0, user->osc_sigma, user->osc_dphi, user->osc_k0);
		}
	}	
